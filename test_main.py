...
import pytest
from main import greet, farewell, greet_formal, thanks, welcome, greet_loud, bye, say_hi, hello_again
...
def test_say_hi_default():
    assert say_hi() == "Hi, there!"


def test_say_hi_with_name():
    assert say_hi("dev") == "Hi, dev!"


def test_hello_again_default():
    assert hello_again() == "Hello again, friend!"


def test_hello_again_with_name():
    assert hello_again("Alice") == "Hello again, Alice!"
